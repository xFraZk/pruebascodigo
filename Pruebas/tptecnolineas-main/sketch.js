...
//------ Configuracion ------
let AMP_MIN = 0.03; // Lo bajamos mucho para que capte tu voz normal
let AMP_MAX = 1; // Bajamos el techo del volumen también
let AMORTIGUACION = 0.9;

let FREC_MIN = 900;
let FREC_MAX = 2000; // Lo subimos mucho para que abarque todos los picos
...
function getPitch() {
  pitch.getPitch(function(err, frequency) {
    // Solo actualizamos si detecta frecuencia Y la amplitud supera tu AMP_MIN
    if (frequency && haySonido) { 
      gestorPitch.actualizar(frequency);
      console.log(frequency);
    } else {
      // Si hay silencio, empujamos el gestor de vuelta al piso (FREC_MIN)
      gestorPitch.actualizar(FREC_MIN); 
    }
    
    // Lo llamamos de nuevo pero con una pausa (ver punto 2)
    setTimeout(getPitch, 500); 
  });
}
